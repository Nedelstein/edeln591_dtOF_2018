...
void Particle::attract(glm::vec2 toPos, float strength) {
    glm::vec2 diff = toPos - pos;
    float distance = glm::length(diff);
    if (distance != 0){
        glm::vec2 attraction = diff / distance * strength;
        applyForce(attraction);
    }
}

void Particle::applyDistanceDamping(glm::vec2 toPos, float minDist, float maxDist, float minDamp, float maxDamp){
    float distance = glm::length(toPos - pos);
    float strength = ofMap(distance, minDist, maxDist, minDamp, maxDamp, true); // clamp
    applyDampingForce(strength);
}
